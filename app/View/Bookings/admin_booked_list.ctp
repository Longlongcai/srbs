<div class = "booking booked_list">
	<?php if(empty($my_bookings)):?>
		<div class="page-header"><h3>  <?php echo $username['User']['username']?>还没有预订记录！</h3> </div>
	<?php else:?>
		<div class="page-header"><h3> <?php echo $username['User']['username'] ?> 的预订列表 </h3> </div>
		...
		<?php $booked_end_time = 
			...
			<tr >
				...
				<td > <?php echo $this->Form->postLink($this->Html->tag('span', '', array('class' => 'glyphicon glyphicon-minus-sign')). ' 取消',array('action' => 'cancel','admin'=>false,$my_booking['Booking']['id']),array('escape'=>false,'class'=>'btn  btn-danger btn-block'),
				    	"确定要取消吗");?> </td>
				  <?php else:?>
				  <td > <?php echo $this->Form->postLink($this->Html->tag('span', '', array('class' => 'glyphicon glyphicon-trash')). ' 过期删除',array('action' => 'cancel','admin'=>true,$my_booking['Booking']['id']),array('escape'=>false,'class'=>'btn  btn-default btn-block'),
				    	"确定要删除吗");?> </td>
				 <?php endif;?>
			</tr>	
			<?php endforeach; ?>
